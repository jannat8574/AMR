...
data <- read_xlsx("AMR/Clean data/AMR_Parental_KAP.xlsx")
#Table 3.  Level of knowledge, attitudes, and practices
data <- data|>
  select(69:71)|>
  tbl_summary() |>
  as_gt()|>
  gtsave("AMR/Table/Table3_Level_of_knowledge,_attitudes,_and_practices.docx")
...
data |> 
  ...
  bold_p(t=0.05)|>
  as_gt() |>
  gtsave("AMR/Table/Table_4_UV_LogReg_Knowledge_level.docx")
...
temp<- data |> 
  ...
  bold_p(t=0.05)|>
  as_gt()|>
  gtsave("AMR/Table/Table_5_UV_LogReg_Attitude_level.docx")


#Figure

data <- read_xlsx("AMR/RAW data/DATA.xlsx")
...
ggsave("AMR/Figure//Figure1.antibiotic_resistance_knowledge.png", 
       plot = plot1, 
       width = 10, 
       height = 6, 
       dpi = 600)
...
ggsave("AMR/Figure/Figure2.Attitude_towards_antibiotic_resistance.png", 
       plot = plot2, 
       width = 15, 
       height = 8, 
       dpi = 600)
...
ggsave("AMR/Figure/Figure3.Practices_among_parents.png", 
       plot = plot3, 
       width = 15, 
       height = 8, 
       dpi = 600)
